...
def diagnostic_differences(ds1, ds2, T=10):
    ...
    for spec in ['KEspec', 'Ensspec']:
        for z in [0,1]:
            if 'time' in ds1[spec].isel(lev=z).coords:
                spec1 = ds1[spec].isel(lev=z, time=-1).mean('run')
            else:
                spec1 = ds1[spec].isel(lev=z).mean('run')
            if 'time' in ds2[spec].isel(lev=z).coords:
                spec2 = ds2[spec].isel(lev=z, time=-1).mean('run')
            else:
    ...
    for spec in ['KEflux', 'APEflux']:
        if 'time' in ds1[spec].coords:
            spec1 = (ds1[spec] + ds1[f"paramspec_{spec}"]).isel(time=-1).mean('run')
        else:
            spec1 = (ds1[spec]+ds1[f"paramspec_{spec}"]).mean('run')
        if 'time' in ds2[spec].coords:
            spec2 = (ds2[spec] + ds2[f"paramspec_{spec}"]).isel(time=-1).mean('run')
        else:
    ...
    for spec in ['APEgenspec', 'KEfrictionspec']:
        if 'time' in ds1[spec].coords:
            spec1 = ds1[spec].isel(time=-1).mean('run')
        else:
            spec1 = ds1[spec].mean('run')
        if 'time' in ds2[spec].coords:
            spec2 = ds2[spec].isel(time=-1).mean('run')
        else:
