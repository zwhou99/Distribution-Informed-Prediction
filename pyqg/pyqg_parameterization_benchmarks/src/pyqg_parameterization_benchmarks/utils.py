...
def energy_budget_term(model, term):
    # 20250423: I find that this function will change the global variable, thus I need deepcopy
    if term == 'KEspec1':
        val = model['KEspec'].isel(lev=0).copy(deep=True)
    elif term == 'KEspec2':
        val = model['KEspec'].isel(lev=1).copy(deep=True)
    elif term == 'Ensspec1':
        val = model['Ensspec'].isel(lev=0).copy(deep=True)
    elif term == 'Ensspec2':
        val = model['Ensspec'].isel(lev=1).copy(deep=True)
    else:
        val = model[term].copy(deep=True)
    if 'paramspec_' + term in model:
        val += model['paramspec_' + term]
    # 20250410: I find that for 'KEflux','APEflux','APEgenspec','KEfrictionspec' from FCNN results,
    # it will have one more dimension time, thus I need to change the original return values
    if 'time' in val.coords and 'run' in val.dims:
        return val.isel(time=-1).mean('run').sum('l')
    elif 'time' in val.coords:
        return val.isel(time=-1).sum('l')
    elif 'run' in val.dims:
        return val.mean('run').sum('l')
    else:
        return val.sum('l')

def energy_budget_figure(models, colors=None, linestyles=None, linewidths=None,
                         skip=1, end=33, layout="1x4"):

    terms = ['KEflux','APEflux','APEgenspec','KEfrictionspec']
    ...
    if layout == "1x4":
        ...
        raise ValueError("layout must be '1x4' or '2x2'")

    vmax = 0

    for i, term in enumerate(terms):
        ax = plt.subplot(nrows, ncols, i + 1)
        ax.set_title(term, fontsize=24)
        ...
        for j in range(len(models)):
            ...
            ax.semilogx(model.k[skip:end], spec[skip:end], label=label,
                        color=None if colors is None else colors[j],
                        lw=2 if linewidths is None else linewidths[j],
                        ls=('--' if '+' in label else '-') if linestyles is None else linestyles[j],
                        alpha=0.9)

            vmax = max(vmax, np.abs(spec[skip:end]).max())

        ax.yaxis.get_offset_text().set_fontsize(18)
        ax.grid(which='both', alpha=0.25)

        if i % ncols == 0:
            ax.set_ylabel("Energy transfer $[m^2 s^{-3}]$", fontsize=22)
        if i % ncols != 0:
            ax.set_yticklabels([])

        ax.set_xlabel("Zonal wavenumber $[m^{-1}]$", fontsize=22)
...
def spectrum_figure(models, colors=None, linestyles=None, linewidths=None,
                    skip=1, end=33, layout="1x4"):

    terms = ['KEspec1', 'KEspec2', 'Ensspec1', 'Ensspec2']
    ...
    for i, term in enumerate(terms):
        ...
        if i % ncols == 0:
            ax.set_ylabel("Power spectrum $[m^2 s^{-2}]$", fontsize=18)

        if i == len(terms) - 1:
            ax.legend()

        ax.set_xlabel("Zonal wavenumber $[m^{-1}]$", fontsize=18)
...
def KEspec_figure(models, skip=0):
    fig = plt.figure(figsize=(10,5))
    terms = ['KEspec1', 'KEspec2']
    for i in range(len(terms)):
        plt.subplot(1, len(terms), i + 1, title=terms[i])
        plt.axhline(0, color='gray', ls='--')
        for model, label in models:
            spec = energy_budget_term(model, terms[i])
            plt.semilogx(model.k[skip:], np.log(spec[skip:]),
                         label=label, lw=3, ls=('--' if '+' in label else '-'))
        plt.grid(which='both', alpha=0.25)
        plt.xlabel("Zonal wavenumber $[m^{-1}]$")
        plt.ylabel("log10(KE spectrum $[m^2 s^{-3}]$)")
    plt.legend()
    plt.tight_layout()
    return fig
